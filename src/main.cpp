...
int main() {
    ...
    while (true) {
        char lenBuf[4];
        if (recvAll(clientSocket, lenBuf, 4) <= 0) break;
        ...
        char msg[msg_length];
        if (recvAll(clientSocket, msg, msg_length) <= 0) break;
        ...
        if (api_key == 18) {
            if (api_version <= 2) {
                ...
                if (send(clientSocket, response, 20, 0) == -1) break;
            } else {
                char response[23];
                ...
                if (send(clientSocket, response, 23, 0) == -1) break;
            }
       } else {
            break;
       }
